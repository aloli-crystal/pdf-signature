...
module PDF
  module Signature
    # Callable CLI entry point for `pdf-sign`. Exposing the command as a
    # method (rather than top-level code in `src/cli.cr`) lets it run both
    # as the standalone `pdf-sign` binary AND in-process from the unified
    ...
    module Cli
      ...
      # (Crystal forbids `return` there). Caught in `.run`.
      ...
      def self.run(argv : Array(String)) : Int32
        ...
        parser = OptionParser.new do |opt|
          ...
          opt.on("-A", "--archive-dss", "B-LTA : enrichir le /DSS du certificat de la TSA d'archive (5e révision)") { archive_dss = true }
        ...
        case subcommand = positional.first?
        when "version"
          puts "pdf-sign #{PDF::Signature::VERSION}"
          0
        when "sign"
      ...
      # PAdES level string → Symbol (raises Halt(1) on an unknown level).
      private def self.parse_level(raw : String) : Symbol
